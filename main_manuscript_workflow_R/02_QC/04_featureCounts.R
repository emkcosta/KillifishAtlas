...
sampleTable <- read.csv(paste0(cwd,"/Output/ExperimentDesign_allbatches_combined_v4.csv"), row.names = 1)  
...
ft.ct1 <- read.delim(paste0(cwd,'/Input/multiqc_featureCounts_batch1.txt'), header = T, sep = '\t')
ft.ct2 <- read.delim(paste0(cwd, '/Input/multiqc_featureCounts_batch2.txt'), header = T, sep = '\t')
ft.ct3 <- read.delim(paste0(cwd,'/Input/multiqc_featureCounts_batch3.txt'), header = T, sep = '\t')
ft.ct4 <- read.delim(paste0(cwd,'/Input/multiqc_featureCounts_batch4.txt'), header = T, sep = '\t')
...
write.csv(ft.ct1.summary, paste0(cwd,"/Output/featurecountsummary_batch1_240424.csv"))

ft.ct3$SampleID <- NA
ft.ct3$SampleID <- str_split_fixed(ft.ct3$Sample, "_",2)[,1]
ft.ct3.summary <- ft.ct3 %>% group_by(SampleID) %>%
  summarise(percent_assigned)
write.csv(ft.ct3.summary, paste0(cwd,"/Output/featurecountsummary_batch3_240424.csv"))
...
ft.ct2.summary <- ft.ct2 %>% group_by(SampleID) %>%
  summarise(percent_assigned)
write.csv(ft.ct2.summary, paste0(cwd,"/Output/featurecountsummary_batch2_240424.csv"))

ft.ct4$SampleID <- NA
ft.ct4$SampleID <- str_split_fixed(ft.ct4$Sample, "_",2)[,1]
ft.ct4.summary <- ft.ct4 %>% group_by(SampleID) %>%
  summarise(percent_assigned)
write.csv(ft.ct4.summary, paste0(cwd,"/Output/featurecountsummary_batch4_240424.csv"))
...
write.csv(sampleTable, paste0(cwd,"/Output/ExperimentDesign_allbatches_combined_v5.csv"))

#Load the raw count matrix
countdata <- read.csv(paste0(cwd,"/Output/Counts_Atlas_allbatches_merged_v2.csv"), row.names = 1)  
...
write.csv(countdata, paste0(cwd,"/Output/Counts_Atlas_allbatches_merged_v3.csv"))

#Load the TPM count matrix
tpm_df <- read.csv(paste0(cwd,"/Output/TPM_Atlas_allbatches_merged_v2.csv"), row.names = 1)
...
write.csv(tpm_df, paste0(cwd,"/Output/TPM_Atlas_allbatches_merged_v3.csv"))
