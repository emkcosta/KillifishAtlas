...
tpm <- read.csv(paste0(cwd,'/Output/TPM_Atlas_allbatches_merged_v3.csv'), stringsAsFactors = FALSE, header = TRUE, check.names = FALSE, row.names = 1)
metadata <- read.csv(file=paste0(cwd,"/Output/ExperimentDesign_allbatches_combined_v7.csv"), stringsAsFactors = FALSE, header = TRUE, check.names = FALSE, row.names = 1 )
...
object.indir = "Path/to/DESeq2_Robjects/" #these files are generated in the 03_DESeq2 directory
...
dir.create(file.path(outdir, subDir2), showWarnings = FALSE)
