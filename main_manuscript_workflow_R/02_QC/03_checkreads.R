...
sampleTable <- read.csv(paste0(cwd,"/Input/ExperimentDesign_allbatches_combined_v3.csv"), row.names = 1)  #this will have been moved from the Output folder
...
rd.ct1 <- read.delim(paste0(cwd,"/Input/mqc_fastqc_sequence_counts_plot_1_Batch1.txt"), header = T, sep = '\t')
rd.ct2 <- read.delim(paste0(cwd,'/Input/mqc_fastqc_sequence_counts_plot_1_batch2.txt'), header = T, sep = '\t')
rd.ct3 <- read.delim(paste0(cwd,'/Input/mqc_fastqc_sequence_counts_plot_1_batch3.txt'), header = T, sep = '\t')
rd.ct4 <- read.delim(paste0(cwd,'/Input/mqc_fastqc_sequence_counts_plot_1_batch4.txt'), header = T, sep = '\t')
...
write.csv(rd.ct1.summary, paste0(cwd,"/Output/readcountsummary_batch1_240425.csv"))
...
rd.ct3.summary <- rd.ct3 %>% group_by(SampleID) %>%
  summarise(sum(Total.Reads))
write.csv(rd.ct3.summary, paste0(cwd,"/Output/readcountsummary_batch3_240422.csv"))
...
write.csv(rd.ct.plate1, paste0(cwd,"/Output/readcountsummary_plate1_allseqbatches_240425.csv"))
...
rd.ct2.summary <- rd.ct2 %>% group_by(SampleID) %>%
  summarise(sum(Total.Reads))
write.csv(rd.ct2.summary, paste0(cwd,"/Output/readcountsummary_batch2_240425.csv"))
...
rd.ct4.summary <- rd.ct4 %>% group_by(SampleID) %>%
  summarise(sum(Total.Reads))
write.csv(rd.ct4.summary, paste0(cwd,"Output/readcountsummary_batch4_240425.csv"))
...
write.csv(rd.ct.plate2, paste0(cwd,"/Output/readcountsummary_plate2_allseqbatches_240425.csv"))
...
write.csv(sampleTable, paste0(cwd,"/Output/ExperimentDesign_allbatches_combined_v4.csv"))

#Load the raw count matrix
countdata <- read.csv(paste0(cwd,"/Output/Counts_Atlas_allbatches_merged.csv"), row.names = 1)  
...
write.csv(countdata, paste0(cwd,"/Output/Counts_Atlas_allbatches_merged_v2.csv"))

#Load the TPM count matrix
tpm_df <- read.csv(paste0(cwd,"/Output/TPM_Atlas_allbatches_merged.csv"), row.names = 1)
...
write.csv(tpm_df, paste0(cwd,"/Output/TPM_Atlas_allbatches_merged_v2.csv"))
