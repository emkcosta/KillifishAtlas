...
Cluster_GOBP <- list()
for(clust in 1:10){
  Cluster_GOBP_temp <- read.delim(paste0("Output/Tables/GOBP_250101_Atlas_",tissue,"hClust",clust,".txt"), row.names = 1)
...
for(clust in 1:10){
  Cluster_Gene_GOBP_temp <- read.delim(paste0("Output/Tables/GeneGOBP_240101_Atlas_",tissue,"hClust",clust,".txt"), header = F)
...
# ------------------------------------------------------------------
clusters <- read.csv(paste0('Output/Tables/Atlas_hclust_loessfits_clusterdata_',tissue,'_confinterval_241231.csv'), row.names =1)
...
# save table
write.table(GOBP_allclust_go, file = 'Output/Tables/250102_Brain_topGOterms-per-hClust_genes.csv', sep = ",", row.names = FALSE)
