...
load('/Output/robjects/dds_TPS_alltissue_byagebin-sexdiffs_DESeq2results_241122.bin')
tissue.list <- names(results_list) #no eye bc there wasnt enough sex balance
tissue.list <- tissue.list[-13]

#---------------- Output all DEGs  ----------------
all_results <- map_dfr(names(results_list), function(tissue) {
  map_dfr(names(results_list[[tissue]]), function(age_bin) {
    map_dfr(names(results_list[[tissue]][[age_bin]]), function(contrast) {
      
      res <- results_list[[tissue]][[age_bin]][[contrast]]$ressig
...
all_results <- all_results %>% filter(contrast == 'M_vs_F')
all_results$gene_symbol = NULL

write.csv(all_results,paste0(outdir.deseq,'250920_M-vs-F_DESeq2_results_alltissues.csv'), row.names = T )
...
#express as % expressed - you need to run correlation for this
outdir.cor = "../04_Correlation/Output/"
